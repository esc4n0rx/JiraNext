...
import { processAvariasIssue } from '@/lib/jira-utils';
import { JiraIssue } from '@/types/jira';

const AVARIAS_JQL = 'project = LOG AND "Request Type" = "Informar avaria na entrega - Central de Produção" AND "Centro de Distribuição - Central de Produção" = RJ ORDER BY created DESC, priority DESC';
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        sendProgress(0, 'Iniciando extração de avarias...');
        const now = new Date().toISOString();
        const { data: extraction, error: extractionError } = await supabaseAdmin.from('extractions').insert({ type: 'avaria', status: 'processing', jql_query: AVARIAS_JQL, start_date: now, end_date: now }).select('id').single();
        ...
        while (!isLast) {
          ...
          const body = JSON.stringify({
            ...
            startAt,
            maxResults: 100,
            // Adicionado 'created' e 'status' aos fields
            fields: ['key', 'created', 'status', 'customfield_10475', 'reporter', 'customfield_10169', 'customfield_11090', 'customfield_10315', 'customfield_11091', 'customfield_10329', 'customfield_11092', 'customfield_10346', 'customfield_11093', 'customfield_10347', 'customfield_11094', 'customfield_10349', 'customfield_10288'],
          });
        ...
        const processedData = allIssues.flatMap(processAvariasIssue);
        ...
        if (processedData.length > 0) {
            const dataToInsert = processedData.map(item => ({
                ...
                created_date: item['Criado'], // Manter o campo antigo se necessário ou remover
            ...
            const { error: insertError } = await supabaseAdmin.from('extraction_avarias').insert(dataToInsert);
        ...
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'complete', totalIssues: allIssues.length, downloadUrl: `/api/jira/avarias/download/${extractionId}`, fileName: `avarias_jira_${extractionId}.xlsx` })}\n\n`));
      } catch (error) {
